...
export default function OvertimeListScreen() {
  ...
  return <OvertimeListContent navigation={navigation} />;
}

function OvertimeListContent({ navigation }) {
  const { 
    overtimeRequests, 
    loading, 
    error, 
  ...
  // Check if user is HR staff (HR Manager or HR Employee)
  ...
  const handleEdit = (voucherCode) => {
    navigation.navigate('EditOvertime', { id: voucherCode });
  };
  ...
  // Permission helper functions - similar to Overtime.vue
  // Đơn đã duyệt không được phép sửa/xóa (kể cả giám đốc)
  const canEditItem = useCallback((item) => {
    if (!item) return false;
    
    // Chỉ cho phép sửa khi status là "Tạo mới" - không cho phép sửa đơn đã duyệt
  ...
  const canDeleteItem = useCallback((item) => {
    ...
    // Chỉ cho phép xóa khi status là "Tạo mới" - không cho phép xóa đơn đã duyệt
    const isCreatedStatus = item.approveStatus === 'Tạo mới' || 
                           item.approveStatus === 0 || 
                           item.approveStatus === '0';
    if (!isCreatedStatus) return false;
  ...
  const canApproveItem = useCallback((item) => {
    ...
    // HR Employee can approve items in "Chờ duyệt" status (depending on workflow)
    if (isHREmployee()) {
      return true;
    }
      ...
      switch (pendingAction) {
        case 'submit':
          ...
          Alert.alert('Thành công', 'Gửi duyệt thành công', [
            { text: 'OK', onPress: () => loadOvertimeRequests() }
          ]);
          break;
        case 'approve':
        case 'reject':
        case 'return':
          await approveOvertimeRequest(selectedItem.voucherCode, pendingAction, notes);
  ...
  // Filter logic with permissions (giống logic trong Overtime.vue)
  const filteredOvertimeRequests = useMemo(() => {
    ...
    return result;
  }, [overtimeRequests, searchQuery, statusFilter, dateRange]);

  // Pagination logic
  const paginatedData = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    return filteredOvertimeRequests.slice(startIndex, endIndex);
  }, [filteredOvertimeRequests, currentPage, itemsPerPage]);

  const totalPages = Math.ceil(filteredOvertimeRequests.length / itemsPerPage);
  ...
  const renderOvertimeItem = ({ item }) => (
    <View style={styles.card}>
      <TouchableOpacity 
        onPress={() => navigation.navigate('OvertimeDetail', { id: item.voucherCode })} 
      ...
      <View style={styles.container}>
        ...
        <View style={styles.toolbarContainer}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => navigation.navigate('AddOvertime')}
          >
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        contentContainerStyle={filteredOvertimeRequests.length === 0 ? styles.emptyListContainer : null}
        ListFooterComponent={
          filteredOvertimeRequests.length > itemsPerPage ? (
            <Pagination
