...
export default function EditOvertimeScreen() {
  ...
  const { getOvertimeRequestById, updateOvertimeRequest, overtimeRequests, fetchOvertimeRequests, loading: overtimeLoading } = useOvertimeRequest();
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Check for overlapping overtime requests (exclude current request)
      const overtimeOverlaps = checkOverlappingOvertimeRequests(
        ...
        overtimeRequests || [],
        overtimeForms || [],
        formData.voucherCode // Exclude current request
      );
